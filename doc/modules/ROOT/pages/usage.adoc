...
[source,clojure]
----
:plugins [[cider/cider-nrepl "0.62.1"]]
----
...
[source,clojure]
----
{:user {:plugins [[cider/cider-nrepl "0.62.1"]]}}
----

Or (if you know what you're doing) add `cider-nrepl` to your `:dev
:dependencies` vector plus the specific middleware you need to
`:nrepl-middleware` under `:repl-options`:

[source,clojure]
----
:dependencies [[cider/cider-nrepl "0.62.1"]]
...
The full list of available middleware lives in the
`cider.nrepl.middleware/cider-middleware` var and is described in
...
----
clj -Sdeps '{:deps {cider/cider-nrepl {:mvn/version "0.62.1"} }}' -M -m nrepl.cmdline --middleware "[cider.nrepl/cider-middleware]"
----
...
NOTE: ClojureScript 1.12.42 and newer require Java 21+, as they bundle a
Google Closure compiler built for it. Stick to the 1.11.x line if you're on an
older JDK.
