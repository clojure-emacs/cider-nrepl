...
Yes, it does. The support, however, is partial and not all of the functionality is available
for ClojureScript. Ops that only make sense on the JVM reply with a `clojure-only`
status when invoked in a ClojureScript session.
