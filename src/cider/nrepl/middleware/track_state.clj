...
(def ^:private ^:dynamic *real-metadata-cache* nil)
(def ^:private ^:dynamic *old-project-state* nil)
...
(defn- get-metadata-if-changed?
  ...
  (let [var-name (.sym the-var)
        ...
        ;; (session) thread.
        cached-meta (.get real-metadata-ns-cache var-name)
        current-meta (meta the-var)]
    (when-not (identical? cached-meta current-meta)
      (.put real-metadata-ns-cache var-name current-meta)
      current-meta)))
...
;; project namespaces both on cider-nrepl and CIDER side. This list is smaller
;; than the one used by `ns-*` middleware.

(def ^:private relevant-meta-keys
  [:deprecated :macro :indent :style/indent :cider/instrumented
...
(def clojure-core-map
  (when clojure-core
    (let [throwaway-whm (WeakHashMap.)]
      {:aliases {}
       :interns (into {}
                      (keep (fn [[k v]]
                              (when (var? v)
                                [k (compute-var-meta v throwaway-whm {})])))
                      (ns-map clojure-core))})))

;;; Clojurescript part

;; NB: must be bound for ClojureScript-related parts of the middleware to work!
(def ^:private ^:dynamic *cljs* nil)
(def ^:private ^:dynamic *all-cljs-namespaces* nil)
