...
(deftest list-only-smoke-tests
  (testing "only test marked as smoke is listed when list-all is run"
    (let [test-result (session/message {:op            "list-all-tests"
      ...
      (is (some #{"#'cider.nrepl.middleware.test-filter-tests/a-puff-of-smoke-test"} (mapcat last (:cider.nrepl.middleware.test-filter-tests test-result))) "smoke test var is not returned"))))
