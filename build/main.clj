...
(defn- javac-opts
  "Compile for Java 8, the oldest version cider-nrepl supports. `--release`
  also checks API usage against the Java 8 class library, so a JDK 9+ API
  slipping in fails on every JDK rather than only on the JDK 8 CI jobs, but
  the flag itself only exists on JDK 9+. Newer JDKs warn that 8 is obsolete;
  targeting it is the whole point here, so that lint category is switched off."
  []
...
(def ^:private antora-file "doc/antora.yml")
...
(defn- set-antora-version
  "Set the Antora docs `version:` key to `value`. Antora versions are
  major.minor (e.g. \"0.62\") on a release tag, and `~` on master so the docs
  build as the development/latest version."
  [contents value]
  (str/replace-first contents #"(?m)^version:.*$" (str "version: " value)))
...
(defn release
  ...
  usage docs, stamp the Antora docs version, commit the result and create the
  annotated `vX.Y.Z` tag. A follow-up commit resets the Antora version back to
  the development default, so the tag captures the versioned docs while master
  keeps building as the latest.
  ...
  (let [version (str version)
    ...
    (let [date (str (java.time.LocalDate/now))]
      ...
      (spit antora-file (set-antora-version (slurp antora-file) minor-version)))
    ...
    (b/git-process {:git-args ["commit" "-m" version
                               changelog-file usage-file ops-file antora-file]})
    (b/git-process {:git-args ["tag" "-a" tag "-m" version]})
    ;; Reset the Antora version on master as a separate, post-tag commit so the
    ;; tagged docs stay pinned to this release.
    (spit antora-file (set-antora-version (slurp antora-file) "~"))
    (b/git-process {:git-args ["commit" "-m" "Reset antora.yml" antora-file]})
    (log "Prepared release %s." version)
    (log "Review the commits, then push to deploy: git push origin HEAD %s" tag)))
